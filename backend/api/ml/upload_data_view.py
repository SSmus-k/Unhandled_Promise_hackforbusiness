import csv
from io import TextIOWrapper
from django.views.decorators.csrf import csrf_exempt
from django.http import JsonResponse
from api.models import CompanyData

@csrf_exempt  # ⚠️ Only for development! Use proper CSRF handling in production.
def upload_csv_view(request):
    if request.method == 'POST' and request.FILES.get('csv_file'):
        csv_file = request.FILES['csv_file']

        try:
            decoded_file = TextIOWrapper(csv_file, encoding='utf-8')
            reader = csv.DictReader(decoded_file)

            for row in reader:
                CompanyData.objects.create(
                    name=row.get('name') or "Unknown",
                    sector=row.get('sector') or "Unknown",
                    is_sustainable=str(row.get('is_sustainable')).lower() in ['1', 'true', 'yes'],
                    has_problem=str(row.get('has_problem')).lower() in ['1', 'true', 'yes'],
                    waste_type=row.get('waste_type') or "",
                    waste_amount=float(row.get('waste_amount') or 0),
                    predicted_waste_next_year=float(row.get('predicted_waste_next_year') or 0),
                    yearly_produced_waste=float(row.get('yearly_produced_waste') or 0),
                    reduced_waste_due_to_recommendation=float(row.get('reduced_waste_due_to_recommendation') or 0),
                )

            return JsonResponse({'status': 'success', 'message': 'CSV uploaded and data saved successfully.'}, status=200)

        except Exception as e:
            return JsonResponse({'status': 'error', 'message': f"Failed to process CSV: {str(e)}"}, status=400)

    return JsonResponse({'status': 'error', 'message': 'No file uploaded or invalid request method.'}, status=400)
